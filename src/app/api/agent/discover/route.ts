...
import { agentDiscoverRequestSchema } from "@/domain/agents/schemas";
...
export async function POST(request: NextRequest): Promise<NextResponse> {
  ...
  try {
    ...
    const parsed = agentDiscoverRequestSchema.safeParse(payload);
    ...
    const allowed = await isAgentRateAllowed("discovery", env.AGENT_RATE_LIMIT_PER_MINUTE ?? 60, 60);

    if (!allowed) {
      return jsonError("Discovery agent rate limit exceeded. Retry in about a minute.", 429);
    }
    ...
    const started = await startAgentRun({
      ...
      inputPayload: {
        ...
        records_count: parsed.data.records.length
      },
    ...
    logger.info("Agent discovery run completed", {
      runId,
      source: parsed.data.source,
      processed: summary.processed,
      inserted: summary.inserted,
    ...
    if (runId) {
      try {
        ...
        logger.error("Failed to mark discovery run as failed", {
          runId,
    ...
    logger.error("POST /api/agent/discover failed", {
      error: error instanceof Error ? error.message : String(error)
    });

    return jsonError("Unable to process agent discovery batch.", 500);
  }
}
